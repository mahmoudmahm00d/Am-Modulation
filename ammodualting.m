...
famlc = (fftshift(fft(amlc)));
fdmlc = (fftshift(fft(dmlc)));
...
subplot(211)
plot(freq,abs(famlc)/N)
title('F-Domain Modulated Signal');  
xlabel('Frequency');
ylabel('Magnitude');
subplot(212)
plot(freq,abs(fdmlc)/N);
title('F-Domain Demodulated Signal');  
xlabel('Frequency');
ylabel('Magnitude');

%% Verifying the output
x = input('Press Enter to play original and demodulated sound:'); 
